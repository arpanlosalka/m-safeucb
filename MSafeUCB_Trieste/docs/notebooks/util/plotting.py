...
def plot_surface(xx, yy, f, ax, contour=False, fill=False, alpha=1.0):
    ...
    if contour:
        if fill:
            return ax.contourf(xx, yy, f.reshape(*xx.shape), 40, alpha=alpha)
        else:
            return ax.contour(xx, yy, f.reshape(*xx.shape), 40, alpha=alpha)
    else:
...
def plot_function_2d(
    ...
    for k in range(F.shape[1]):
        ...
        # plt_obj = plot_surface(xx, yy, f, axx, contour=contour, fill=fill, zorder=1)
...
# Function added for drawing safe boundary

def plot_safe_boundary_2d(
        obj_func,
        ax,
        mins: TensorType,
        maxs: TensorType,
    ...
    """
    Plot of safe boundary as estimated by the algorithm
    :param obj_func: a function that returns a n-array given a [n, d] array
    :param mins: 2 lower bounds
    :param maxs: 2 upper bounds
    :param thres: safety threshold
    :param grid_density: positive integer for the grid size
    :param title:
    :param xlabel:
    :param ylabel:
    ...
    for i in range(grid_density):
        j = 1
        while F[i, j - 1] <= thres and F[i, j] <= thres:
            j += 1
            if j == grid_density:
                break
        max_x.append(xx[0][j - 1])

    y = np.linspace(mins[1], maxs[1], grid_density)
    ax.plot(max_x, y, c=plot_color, marker=plot_marker, alpha=alpha, zorder=10, markevery=markevery)
    return ax
...
def plot_bo_points(
    ...
    if obs_values is None:
        for i in range(pts.shape[0]):
            ax.scatter(pts[i, 0], pts[i, 1], c=col_pts[i], marker=mark_pts[i], zorder=10)
    else:
        for i in range(pts.shape[0]):
            ax.scatter(pts[i, 0], pts[i, 1], obs_values[i], c=col_pts[i], marker=mark_pts[i], zorder=10)
...
def plot_cumulative_regret(
    ...
    cum_values_t = [cum_values[i] / (i + 1) for i in range(80)]
    ax.plot(cum_values_t, color="tab:orange")
